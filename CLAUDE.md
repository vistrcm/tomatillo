...
A macOS pomodoro focus timer that lives in the menu bar. When the work timer ends, a fullscreen "curtain" window covers all displays to enforce a break. The break timer counts down visually (binary dot display), then automatically starts the next work session.
...
- **`TomatilloTimer.swift`** — `ObservableObject` with `@Published isRunning` and `remaining`. 1-second `Timer.scheduledTimer` countdown. Reused for both work timer (in `TomatilloApp`) and break timer (in `CurtainController`). Calls `onFinished` when countdown hits zero.
...
- **`CurtainView.swift`** — The largest file, contains:
  ...
  - `CurtainController` (singleton) — manages the full curtain lifecycle:
    ...
    - `snooze(wallpaper:)` — hides curtain, re-shows after `snoozeDuration` with same wallpaper and `snoozed: true` (hides snooze button).
  ...
  - `CurtainContent` — break screen UI: binary dot timer (circles lit/dim), hover reveals M:SS text. Buttons: Snooze (one-time, hidden after use), Next (skip to work), Lock Screen.
...
Work timer ends → show() curtain on all screens → break timer starts
        ...
        [Lock]   → lock macOS screen                     |
                                                         ↓
                                            Break ends → onNext → restart work timer
                                                         (automatic loop)
```
